...
class Notificationpush():
    ...
    def send_notification(data):
        ...
        success = Notificationpush.send_push_notification(device_token, title, body)
        if success:
            return jsonify({
                "status": True,
...
class Filterations:
    ...
    def filter_events_by_location(events_data, user_location, max_distance=None):
        ...
        for event in events_data:
            event_latitude = event.get("event_latitude")
            event_longitude = event.get("event_longitude")

            if Filterations.is_valid_latitude(event_latitude) and Filterations.is_valid_longitude(event_longitude):
                event_location = (event_latitude, event_longitude)
                distance = Filterations.calculate_distance(user_location, event_location)
